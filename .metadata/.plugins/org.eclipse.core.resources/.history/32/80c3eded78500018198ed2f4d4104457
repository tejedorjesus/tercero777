...
public class GestionColas {
	...
	public static void main(String[] args) {
		...
		for (t = 1; t <= HORAS * 60; t++) {
			// caso de entrar un cliente medimos colas lo generamos y le
			// asignamos ventanilla
			...
			// calculo de las longitudes de cada cola en cada instante y se
			// acumulan
		...
		for (indice = 0; indice < NUM_VENTANILLAS; indice++) {
			Leer.mostrarEnPantalla("longitud media de la cola:" + (indice + 1) + ":"
					+ (double) longitudesFilas[indice] / (HORAS * 60));
		}
		suma = 0;
		for (indice = 0; indice < NUM_VENTANILLAS; indice++) {
	...
	public static int medir(List<Integer> ventanillas[]) {
		...
		/***********************************************************
		 * devuelve min, que sera el num de ventanilla con menos gente es decir,
		 * donde insertaremos la tarea
		 ***********************************************************/
		int min, k;
		min = 0;
